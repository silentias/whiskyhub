...
DEFAULT_COMMANDS_PATH = Config.COMMANDS_PATH
...
class CommandCatalog:
    ...
    def _load(self) -> tuple[CommandDefinition, ...]:
        ...
        template_path = Config.COMMANDS_TEMPLATE_PATH
        if self.path.resolve() != template_path.resolve():
            ...
            rows.extend(
                row
                for row in self._load_rows(template_path)
                if isinstance(row, dict) and row.get("slug") not in existing_slugs
            )
    ...
    def _load_rows(path: Path) -> list[object]:
        if not path.is_file():
            raise FileNotFoundError(f"Файл команд не найден: {path}")

        try:
            document = yaml.safe_load(path.read_text(encoding="utf-8"))
        except yaml.YAMLError as error:
            raise ValueError(f"Некорректный YAML в {path}: {error}") from error
